...
<?php
	...
	$sql="SELECT CÓDIGOARTÍCULO, SECCIÓN, PRECIO, PAÍSDEORIGEN FROM PRODUCTOS WHERE PAÍSDEORIGEN=?"; // Se debe incluir ? para realizar la consulta preparada (paso 1)
	
	$resultado=mysqli_prepare($conexion,$sql); // se almacena el objeto mysqli_smtp, se preapara la consulta (paso 2)
	$ok= mysqli_stmt_bind_param($resultado, "s", $pais); //se almacena true o false (paso 3)
